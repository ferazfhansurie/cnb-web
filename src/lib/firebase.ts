...
export async function updateUserRole(userId: string, newRole: 'Admin' | 'Manager' | 'User - Price' | 'User - No Price' | 'Pending') {
  // Verify the current user has permission to update roles
  if (auth.currentUser) {
    const currentUserDoc = await getDoc(doc(usersCollection, auth.currentUser.uid));
    const currentUserData = currentUserDoc.data();
    
    if (!currentUserData || (currentUserData.role !== 'Admin' && currentUserData.role !== 'Manager')) {
      throw new Error('Only administrators and managers can modify user roles');
    }
  }
